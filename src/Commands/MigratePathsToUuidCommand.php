...
class MigratePathsToUuidCommand extends Command
{
    use ConfirmableTrait;

    protected $signature = 'media-library:migrate-paths-to-uuid
    ...
    protected $description = 'Migrate media files from DefaultPathGenerator (ID-based) to the configured UUID path generator.';

    private DefaultPathGenerator $oldGenerator;

    private AbstractUuidPathGenerator $newGenerator;
    ...
    public function handle(Factory $fileSystem): void
    {
        ...
        $this->newGenerator = $this->resolveUuidGenerator();
     ...
     * generators — migrating into anything else would silently scatter files
     * to wherever the foreign generator decides.
     */
    private function resolveUuidGenerator(): AbstractUuidPathGenerator
    {
        ...
        if (! is_string($generator) || ! is_subclass_of($generator, AbstractUuidPathGenerator::class)) {
            throw new RuntimeException(
                ...
                . ' before running media-library:migrate-paths-to-uuid.'
            );
        }
